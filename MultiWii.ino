...
void setup() {
  ...
  //UAVIONICS
  #if defined (USE_ARDUINO_SERIAL)
  SerialOpen(0,38400); //to communicate RFID with 38400 baud rate
  #endif
   //end of modified block
    #if defined(PROMICRO)
...
void loop () {
 ...
 digitalWrite(13,HIGH); //Turn on D4 on board LED without arming the quadcopter (Normally arming the quadcopter will turn on the LED and disarming will turn off the LED) 
 #if defined (USE_ARDUINO_SERIAL) //any arduino serial use will have to be inside this #defined and #endif
 //  Serial.print("Hello World"); //uncomment to display "Hello World" on serial terminal
 if (Serial.available() > 0) {   
   int serialIn = Serial.read(); //get serial input and store as integer
   Serial.print (serialIn,HEX); //print serial output as Hex format
   Serial.print(" "); //print space 
 }//end of checking serial input
 #endif //end of defined (USE_ARDUINO_SERIAL)
 }//end of function
 //end of modified block
